...
void gcta::update_id_map_kp(const vector<string> &id_list, map<string, int> &id_map, vector<int> &keep) {
    int i = 0;
    //temporary buffer of id_map
    map<string, int> id_map_buf(id_map);
    
    //remove elements of id_map_buf contained in id_list
    for (i = 0; i < id_list.size(); i++) id_map_buf.erase(id_list[i]);
    
    //what's left over in id_map_buf remove in  
...
void gcta::save_maf() {
    calcu_maf();
    string save_maf = _out + ".maf";
    ofstream out(save_maf.c_str());
    if (!out) throw ("Error: can not open the file [" + save_maf + "] to write.");
    int i = 0;
    cout << "Writing minor allele frequencies of " << _include.size() << " SNPs to [" + save_maf + "]." << endl;

    for (i = 0; i < _include.size(); i++) {
        out << _snp_name[_include[i]] << "\t" << _ref_A[_include[i]] << "\t" << setprecision(15) << _maf[i];
        //        if(ssq_flag) ofreq<<"\t"<<_ssq[_include[i]]<<"\t"<<_w[_include[i]];
        out << endl;
    }
    out.close();
