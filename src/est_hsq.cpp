...
void gcta::construct_X(int n, map<string, int> &uni_id_map, bool qcovar_flag, int qcovar_num, vector<string> &qcovar_ID, vector< vector<string> > &qcovar, bool covar_flag, int covar_num, vector<string> &covar_ID, vector< vector<string> > &covar, vector<eigenMatrix> &E_float, eigenMatrix &qE_float) {
    ...
    /*
    bool maternal_flag = false;

    eigenMatrix X_m;
    // impute maternal genotype
    if (maternal_flag) {
        X_m.resize(n, 1);
        //X_m = eigenMatrix::Ones(n, 1);
        srand(time(NULL));
        for (int i = 0; i < n; i++) 
        {
    ...
    */
    ...
    // row in first column is set to 1
...
bool gcta::calcu_Vi(eigenMatrix &Vi, eigenVector &prev_varcmp, double &logdet, int &iter)
{
    ...
    if (_r_indx.size() == 1) {
        ...
        //todo: find out _A and prev_varcmp
...
{
    Vi_X = Vi*_X;
    //cout << "Vi size: " << Vi.rows() << " " << Vi.cols() << endl;
    //cout << "Vi_X size: " << Vi_X.rows() << " " << Vi_X.cols() << endl;
    cout << "X size: " << _X.rows() << " " << _X.cols() << endl;
    Xt_Vi_X_i = _X.transpose() * Vi_X;
    //cout << "Xt_Vi: " << Xt_Vi_X_i.rows() << " " << Xt_Vi_X_i.cols() << endl;
    //cout << Xt_Vi_X_i(0, 0) << " " << Xt_Vi_X_i(0, 1) << endl;
    //cout << Xt_Vi_X_i(1, 0) << " " << endl;//<< Xt_Vi_X_i(1, 1) << endl;
