...
void gcta::mlma(string grm_file, bool m_grm_flag, string subtract_grm_file, string phen_file, string qcovar_file, string covar_file, int mphen, int MaxIter, vector<double> reml_priors, vector<double> reml_priors_var, bool no_constrain, bool within_family, bool inbred, bool no_adj_covar, bool imp_mat_geno)
{
    ...
    if (imp_mat_geno) no_adj_covar = true;
    ...
    // _y <individual index -> phenotype (float)>
    _y.setZero(_n);
    ...
    if (subtract_grm_flag) {
        ...
        //std::cout << _r_indx.size() << std::endl; 
        _A.resize(_r_indx.size());
        if(grm_flag){
            StrFunc::match(uni_id, grm_id, kp);
            (_A[0]).resize(_n, _n);
            //A is symmetric matrix, kp is matrix of indices 
            #pragma omp parallel for private(j)
            for(i=0; i<_n; i++){
    ...
    //last element is identity matrix
...
/*    if(grm_flag || m_grm_flag){
    ...
    eigenVector beta, beta2, se, se2, pval, pval2;
    // n = no. of individua
    if(no_adj_covar) mlma_calcu_stat_covar(y, _geno_mkl, n, m, beta, beta2, se, se2, pval, pval2 ,imp_mat_geno);
    else mlma_calcu_stat(y, n, m, beta, se, pval);
    delete[] y;
    delete[] _geno_mkl;
    ...
    if (imp_mat_geno) {
        ofile<<"Chr\tSNP\tbp\tA1\tA2\tFreq\tb\tb2\tse\tse2\tp\tp2"<<endl;
    } else {
    ...
    for(i=0; i<m; i++){
        ...
        if(imp_mat_geno) {
            if(pval2[i]>1.5) ofile<<"NA\tNA\tNA\tNA\tNA\tNA\tNA"<<endl;
            else ofile<<0.5*_mu[j]<<"\t"<<beta[i]<<"\t"<<beta2[i]<<"\t"<<se[i]<<"\t"<<se2[i]<<"\t"<<pval[i]<<"\t"<<pval2[i]<<endl;
        } else {
...
void gcta::mlma_calcu_stat(float *y, unsigned long n, unsigned long m, eigenVector &beta, eigenVector &se, eigenVector &pval)
{
    //note: n = no. of individuals, m = no. of SNPs
...
void gcta::mlma_calcu_stat_covar(float *y, float *geno_mkl, unsigned long n, unsigned long m, eigenVector &beta, eigenVector &beta2, 
    eigenVector &se, eigenVector &se2, eigenVector &pval, eigenVector &pval2, bool imp_mat_geno)
{
    int max_block_size = 10000;
    ...
    for(i=0; i<n; i++){
        ...
        X[i*col_num+_X_c+1]=0.0; 
    }

    beta.resize(m);
    beta2.resize(m);
    se=eigenVector::Zero(m);
    se2=eigenVector::Zero(m);
    pval=eigenVector::Constant(m,2);
    pval2=eigenVector::Constant(m,2);
    cout<<"\nRunning association tests for "<<m<<" SNPs ..."<<endl;
    int new_start = 0, block_size = 0, block_col = 0, k = 0, l = 0;
    MatrixXf X_block;
    eigenMatrix mat_geno_block;
    vector<int> indx;
    for(i = 0; i < m; i++, block_col++){
        // get a block of SNPs
        if(i == new_start){
            ...
            // get block of maternal genotypes
            if (imp_mat_geno) {
                mat_geno_block.resize(n, block_size);
                mat_geno_block = _mat_geno.block(0, i, n, block_size);
            }
        }
        ...
        for(j = 0; j < n; j++) 
        {
            ...
            // add maternal genotype column
            if (imp_mat_geno) {
                X[j*col_num+_X_c+1] = mat_geno_block(j, block_col); 
            }
        }
        // Vi_X = Vi * X
        ...
        if(!comput_inverse_logdet_LU_mkl_array(col_num, Xt_Vi_X, d_buf)) {
            printf("Error: Xt_Vi_X is not invertable, caused by: %s\n", _snp_name[i].c_str());
            continue;
            //throw("Error: Xt_Vi_X is not invertable. caused by " + _snp_name[i]);
        }
        // Xt_Vi_y = (Vi * X)^T * Y
        cblas_sgemv(CblasRowMajor, CblasTrans, n, col_num, 1.0, Vi_X, col_num, y, 1, 0.0, Xt_Vi_y, 1);
        // b_vec = (X^T * Vi * X)^(-1) * ((Vi * X)^T * Y)
        ...
        if (imp_mat_geno) {
            beta2[i]=b_vec[_X_c+1];
            se2[i]=Xt_Vi_X[(_X_c+1)*col_num+_X_c+1];
        }
        
        //printf("i: %d B0:%f B1:%f B2:%f B3:%f B4:%f\n", i, b_vec[0], b_vec[1], b_vec[2], b_vec[3], b_vec[4]);
        ...
        if(se[i]>1.0e-30){
            ...
            if (imp_mat_geno) {
                chisq=beta2[i]/se2[i];
                pval2[i]=StatFunc::pchisq(chisq*chisq, 1);  
            }
        }
    }
    delete[] Vi;
...
void gcta::mlma_loco(string phen_file, string qcovar_file, string covar_file, int mphen, int MaxIter, vector<double> reml_priors, vector<double> reml_priors_var, bool no_constrain, bool inbred, bool no_adj_covar)
{
    ...
    for(c1=0; c1<chrs.size(); c1++){
        ...
        mlma_calcu_stat(y, n, _include.size(), beta[c1], se[c1], pval[c1]);
